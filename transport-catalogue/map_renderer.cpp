...
namespace renderer {
    ...
    std::vector<svg::Polyline> MapRenderer::GetRouteLines(const std::map<std::string_view, const trasport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
        ...
        for (const auto& [bus_number, bus] : buses) {
            ...
            std::vector<const trasport_catalogue::Stop*> route_stops{ bus->stops.begin(), bus->stops.end() };
            if (!bus->is_circular) {
    ...
    std::vector<svg::Text> MapRenderer::GetBusLabel(const std::map<std::string_view, const trasport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
        ...
        for (const auto& [bus_number, bus] : buses) {
            ...
            if (bus->is_circular == false && bus->stops[0] != bus->stops[bus->stops.size() - 1]) {
                svg::Text text2 {text};
    ...
    std::vector<svg::Circle> MapRenderer::GetStopsSymbols(const std::map<std::string_view, const trasport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
    ...
    std::vector<svg::Text> MapRenderer::GetStopsLabels(const std::map<std::string_view, const trasport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
        std::vector<svg::Text> result;
        svg::Text text;
        svg::Text underlayer;
        for (const auto& [stop_name, stop] : stops) {
    ...
    svg::Document MapRenderer::GetSVG(const std::map<std::string_view, const trasport_catalogue::Bus*>& buses) const {
        svg::Document result;
        std::vector<geo::Coordinates> route_stops_coord;
        std::map<std::string_view, const trasport_catalogue::Stop*> all_stops;
